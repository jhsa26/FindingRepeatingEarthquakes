...
nevent=8000  # only the first nevent events in file.list_old are used to do cluster
ccthreshold=0.8 
minev=4      # only cluster with more than 5 repeating earthquakes is reserved           
...
maxevent=`ls -1 data_sac/*.SAC | wc -l`
nevent=`echo "$maxevent $nevent" | awk '{if($1<$2){print $1}else{print $2}}'`
echo "number of events to be cluster: ${nevent}"
echo "number of events to be cluster: ${nevent}" >"repeating_cluster.log"
...
############################################################
date;date >>"repeating_cluster.log"
...
./bin/cc2clusterHierarchical  ./output/ccfile $nevent $ccthreshold m |paste - ./output/file.list |\
    ./getcluster.awk  | uniq -w15 -D   > "all_cluster"

#./bin/cc2clusterHierarchical  ./output/ccfile $nevent $ccthreshold m |paste - ./output/file.list |\
#    ./getcluster.awk     > "all_cluster"
rm -rf Cluster/*
